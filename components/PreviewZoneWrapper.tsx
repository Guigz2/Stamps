'use client';

import dynamic from 'next/dynamic';

interface PreviewZoneProps {
  pdfFile: File;
  config: {
    stampType: 'text' | 'image';
    initials: string;
    stampImage: string | null;
    font: string;
    size: number;
    xOffset: number;
    yOffset: number;
    opacity: number;
  };
  onClose: () => void;
  onPdfInfoChange?: (info: { currentPage: number; numPages: number; isProcessing: boolean }) => void;
  onRegisterProcessHandler?: (handler: () => void) => void;
  externalCurrentPage?: number;
  onConfigChange: (newConfig: Partial<PreviewZoneProps['config']>) => void;
}

// Import dynamique pour éviter les erreurs SSR avec pdf.js
const PreviewZoneClient = dynamic<PreviewZoneProps>(
  () => import('./PreviewZone').then(mod => mod.default),
  { 
    ssr: false,
    loading: () => (
      <div className="flex-1 flex flex-col">
        <div className="flex-1 flex items-center justify-center bg-[#0A0E27]">
          <div className="text-center space-y-4">
            <svg className="animate-spin h-16 w-16 text-[#FF6B2C] mx-auto" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="text-white font-semibold">Initializing PDF viewer...</p>
          </div>
        </div>
      </div>
    )
  }
);

export default PreviewZoneClient;
